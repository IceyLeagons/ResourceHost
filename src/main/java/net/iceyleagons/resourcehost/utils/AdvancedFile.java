 ...
 * so no need for complementary methods. (This class is in conjuction with {@link FileUtils})
 *
 * <p>
 ...
 * @since 1.0.0
 */
public record AdvancedFile(File file, boolean folder) {
    ...
    /**
     * Deletes this {@link AdvancedFile} by checking whether it's a directory ({@link #folder()}), if it is {@link FileUtils#deleteFolder(File)} will be called,
     * otherwise Java's default {@link File#delete()} will be used.
     */
    public void delete() {
        if (folder()) FileUtils.deleteFolder(file());
        else file().delete();
    }
    ...
    public AdvancedFile makeExactCopy(File destination, String... ignore) {
        AdvancedFile copy = new AdvancedFile(new File(destination, this.file.getName()), folder());
    ...
    public AdvancedFile createOrGetChild(String name, boolean folder) {
        ...
        return new AdvancedFile(new File(file(), name), folder);
    }
    ...
    public void copyTo(AdvancedFile destination, String... ignore) {
        copyTo(destination.file(), ignore);
    }
    ...
    public void copyTo(File destination, String... ignore) {
        ...
        Asserts.state(destination.isDirectory() == file().isDirectory(), "Current and destination file does not match file type!");
    ...
    public AdvancedFile moveFile(AdvancedFile destination) {
        ...
        AdvancedFile copy = this.makeExactCopy(destination.file());
